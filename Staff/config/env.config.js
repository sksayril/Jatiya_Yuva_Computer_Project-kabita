...
/**
 * Environment Configuration Manager for Staff Panel
 */
const envConfig = {
  // Node Environment
  ...
  // JWT Configuration
  JWT_SECRET: process.env.JWT_SECRET || 'dev_staff_jwt_secret_change_me',
...
try {
  validateEnv();
  console.log('✅ Staff Panel environment variables validated successfully');
} catch (error) {
  console.error('❌ Staff Panel environment validation failed:', error.message);
  if (isProduction()) {
    process.exit(1);
  } else {
