...
/**
 * Environment Configuration Manager for Student Panel
 */
const envConfig = {
  // Node Environment
  ...
  // JWT Configuration
  JWT_SECRET: process.env.JWT_SECRET || 'dev_student_jwt_secret_change_me',
...
try {
  validateEnv();
  console.log('✅ Student Panel environment variables validated successfully');
} catch (error) {
  console.error('❌ Student Panel environment validation failed:', error.message);
  if (isProduction()) {
    process.exit(1);
  } else {
